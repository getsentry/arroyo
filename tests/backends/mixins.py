...
class StreamsTestMixin(ABC, Generic[TStrategyPayload]):
    ...
    def test_consumer(self, with_stmt: WithStmt) -> None:
        ...
        messages = [
            ...
            for future in [producer.produce(topic, next(payloads)) for i in range(2)]
        ]
    ...
    def test_working_offsets(self, with_stmt: WithStmt) -> None:
        payloads = self.get_payloads()
    ...
    def test_pause_resume(self, with_stmt: WithStmt) -> None:
        payloads = self.get_payloads()
        ...
        with assert_does_not_change(consumer.paused, []), pytest.raises(ConsumerError):
    ...
    def test_pause_resume_rebalancing(self, with_stmt: WithStmt) -> None:
        ...
        topic = with_stmt(self.get_topic(2))
        producer = with_stmt(closing(self.get_producer()))
        consumer_a = with_stmt(
            closing(self.get_consumer("group", enable_end_of_partition=False))
        )
        consumer_b = with_stmt(
            closing(self.get_consumer("group", enable_end_of_partition=False))
        )
        ...
        assert consumer_a.poll(10.0) in messages  # XXX: getting the subcription is slow
